...
**Tech Stack:** Next.js (App Router) + TypeScript, Tailwind, Drizzle ORM, Neon Postgres, Auth.js v5 (magic link via Resend), Vitest, MSW, Playwright, pnpm.
...
AUTH_SECRET=
AUTH_RESEND_KEY=
EMAIL_FROM=overdue@example.com
HARDCOVER_API_TOKEN=
```
...
**Files:**
- Create: `src/lib/auth.ts`, `src/app/api/auth/[...nextauth]/route.ts`, `src/app/signin/page.tsx`, `src/middleware.ts`

**Interfaces:**
- Consumes: `db` from Task 2, `users`/`accounts`/`sessions`/`verificationTokens` from Task 3
- Produces: `auth()` returning the current session or `null`; `signIn`/`signOut` server actions
...
describe("auth config", () => {
  it("uses the database session strategy", async () => {
    const { authConfig } = await import("./auth-config");
    expect(authConfig.session?.strategy).toBe("database");
  });

  it("routes unauthenticated users to /signin", async () => {
    const { authConfig } = await import("./auth-config");
...
- [ ] **Step 6: Wire the adapter and export handlers**
...
export default function SignInPage() {
  return (
    <main className="mx-auto flex min-h-dvh max-w-sm flex-col justify-center gap-6 px-6">
      <h1 className="font-[family-name:var(--font-newsreader)] text-3xl">
        Overdue
      </h1>
      ...
      >
        ...
        />
        <button type="submit" className="rounded-sm border px-3 py-2">
          Send sign-in link
        </button>
      </form>
    </main>
...
Add a Resend API key to `.env.local` as `AUTH_RESEND_KEY`, set `EMAIL_FROM`, and generate `AUTH_SECRET`:

```bash
pnpm dlx auth secret
```

Run `pnpm dev`, visit http://localhost:3000, confirm redirect to `/signin`, submit your email, and confirm the magic link signs you in.

- [ ] **Step 10: Commit**

```bash
git add -A
git commit -m "feat: add magic link authentication with Auth.js and Resend"
```
...
Import the repo at https://vercel.com/new, and set these environment variables in the Vercel project: `DATABASE_URL`, `AUTH_SECRET`, `AUTH_RESEND_KEY`, `EMAIL_FROM`, `HARDCOVER_API_TOKEN`.

- [ ] **Step 4: Verify the deployment**

Visit the Vercel production URL, confirm redirect to `/signin`, and confirm a magic link signs you in against the production database.
...
export async function POST(request: Request): Promise<Response> {
  ...
  await insertTrack(session.user.id, {
...
export async function POST(request: Request): Promise<Response> {
  ...
  await insertTrack(session.user.id, { seriesId: null, bookId });
...
test("the sign-in form asks for an email address", async ({ page }) => {
...
```bash
git add -A
git commit -m "test: add Playwright end-to-end coverage for the auth boundary"
git push
```
...
- [ ] The deployed Vercel URL redirects to `/signin` and a magic link signs you in
