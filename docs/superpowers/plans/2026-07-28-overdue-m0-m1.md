...
- **Provider adapters never call `fetch` directly and never cast `res.json()` to a shape.** They use `fetchJson` and the narrowing helpers from `src/providers/http.ts`, which return `null`/`undefined` rather than throwing on a network error, a non-2xx status, invalid JSON, or an unexpected shape. This supersedes the raw `fetch` plus `as` cast shown in the Task 7 to Task 10 sample code, which contradicted the no-`any` constraint and would throw on a malformed 200 response. Added 2026-07-28 after review of Task 7.
